...
from engine import training_step, testing_step, eval_func
...
#   print(f"Epoch: {epoch+1}")
  train_loss, train_acc, _ = training_step(model = central_model, dataloader = train_dataloader,
                                    loss_fn = loss_fn, optimizer = optimizer,
                                    acc_fn= accuracy_fn, device = device)
  
  test_loss, test_acc = testing_step(model = central_model, dataloader = test_dataloader,
                                  loss_fn = loss_fn, acc_fn = accuracy_fn,
                                  device = device)
  
  central_model_train_loss.append(train_loss)
  central_model_test_loss.append(test_loss)
  central_model_train_accs.append(train_acc)
  central_model_test_accs.append(test_acc)
